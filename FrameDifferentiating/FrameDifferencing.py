...
class FrameDifferencing:
    ...
    def get_mask(frame1, frame2, kernel=np.array((9, 9), dtype=np.uint8)):
        ...
        return mask
    def get_contour_detections(mask, thresh=10):
    ...
    def non_max_suppression(boxes, scores, threshold=1e-1):
        ...
        order = FrameDifferencing.remove_contained_bboxes(boxes)
    ...
    def get_detections(frame1, frame2, bbox_thresh=10, nms_thresh=1e-3, mask_kernel=np.array((9, 9), dtype=np.uint8)):
            ...
            mask = FrameDifferencing.get_mask(frame1, frame2, mask_kernel)

            # get initially proposed detections from contours
            detections = FrameDifferencing.get_contour_detections(mask, bbox_thresh)
            ...
            return FrameDifferencing.non_max_suppression(bboxes, scores, nms_thresh)
